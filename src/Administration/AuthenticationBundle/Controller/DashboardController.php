...
class DashboardController extends Controller
{
    public function showAction()
    {
        ...
        //accounts by hour
        $accounts_array = [['Date', 'Accounts', 'Estimated']];
        $previous = null;
        $done = Array();
        $done_total = Array();
        $previous_done = null;
        foreach ($_connections_usage_array as $datum) {
            ...
            if (!isset($done[date("YW", $datum["datesave"])])) {
                if ($previous_done) {
                    $accounts_array[] = [date("d/m/Y H:i", $datum["datesave"]), $done[$previous_done], 0];
                }
                $done[date("YW", $datum["datesave"])] = 0;
                $done_total[date("YW", $datum["datesave"])] = 0;
                $previous_done = date("YW", $datum["datesave"]);
            }
        ...
        //ram
        $data["ram"]["percent"] = $this->get("admin.TwakeServerStats")->getRamUsage();
