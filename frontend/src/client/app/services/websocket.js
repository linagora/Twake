...
class Websocket extends Observable {
  constructor() {
    ...
    this.autobahn = Globals.autobahn;
    ...
    var idleInterval = setInterval(this.alive.bind(this), 30000); // 30 seconds
  }
  ...
  reconnect() {
    ...
    try {
      ...
      if (
        ...
        connection = that.autobahn.connect('wss://' + Globals.window.WEBSOCKET_URL);
      } else {
        connection = that.autobahn.connect('ws://' + Globals.window.WEBSOCKET_URL);
      }
    ...
    if (
      ...
      /*if(!navigator.onLine && this.lastData>0 && (new Date()).getTime() - this.lastData > 2000){
        ...
        try {
          ...
          console.log('Unable to call on ws');
        }
      }
    }
