...
// Last time updated: 2017-08-31 6:18:41 AM UTC
...
window.RTCMultiConnection = function (roomid, forceOptions) {
	function SocketConnection(connection, connectCallback) {
		...
		connection.socket.on("extra-data-updated", function (remoteUserId, extra) {
			...
			if (message.remoteUserId == connection.userid) {
				...
				if ("connectWithAllParticipants" === message.message)return connection.broadcasters.indexOf(message.sender) === -1 && connection.broadcasters.push(message.sender), void mPeer.onNegotiationNeeded({allParticipants: connection.getAllParticipants(message.sender)}, message.sender);
			...
			var eventObject = {userid: userid, extra: {}};
	...
	function PeerInitiator(config) {
		...
		function oldAddRemoteSdp(remoteSdp, cb) {
			cb = cb || function () {
			}, remoteSdp.sdp = connection.processSdp(remoteSdp.sdp), peer.setRemoteDescription(new RTCSessionDescription(remoteSdp), cb, function (error) {
		...
		function createOfferOrAnswer(_method) {
			return window.enableAdapter ? void peer[_method](defaults.sdpConstraints).then(function (localSdp) {
				localSdp.sdp = connection.processSdp(localSdp.sdp), peer.setLocalDescription(localSdp).then(function () {
					connection.trickleIce && (config.onLocalSdp({
						type: localSdp.type,
						sdp: localSdp.sdp,
						remotePeerSdpConstraints: config.remotePeerSdpConstraints || !1,
		...
		peer.onicecandidate = function (event) {
			...
			if (!(config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.dontGetRemoteStream) && !config.dontAttachLocalStream && (localStream = connection.beforeAddingStream(localStream, self)))if (getLocalStreams().forEach && getLocalStreams().forEach(function (stream) {
			...
			}); else if (localStream && "function" == typeof peer.addStream)peer.addStream(localStream); else try {
		...
		var dontDuplicate = {}, incomingStreamEvent = "track";
		window.enableAdapter || (incomingStreamEvent = "addstream"), peer.addEventListener(incomingStreamEvent, function (event) {
			...
			}, window.enableAdapter ? (remoteSdp.sdp = connection.processSdp(remoteSdp.sdp), void peer.setRemoteDescription(new RTCSessionDescription(remoteSdp)).then(cb, function (error) {
				connection.enableLogs && console.error("setRemoteDescription failed", "\n", error, "\n", remoteSdp.sdp), cb()
	...
	function getUserMediaHandler(options) {
		function streaming(stream, returnBack) {
			...
			}, !1), currentUserMediaRequest.streams[idInstance] = {stream: stream}, currentUserMediaRequest.mutex = !1, currentUserMediaRequest.queueRequests.length && getUserMediaHandler(currentUserMediaRequest.queueRequests.shift())
		}
		...
		if (currentUserMediaRequest.streams[idInstance])streaming(currentUserMediaRequest.streams[idInstance].stream, !0); else {
			...
			if (window.enableAdapter || "Safari" !== DetectRTC.browser.name || (options.localMediaConstraints.audio !== !1 && (options.localMediaConstraints.audio = !0), options.localMediaConstraints.video !== !1 && (options.localMediaConstraints.video = !0)), "undefined" == typeof navigator.mediaDevices) {
			...
			navigator.mediaDevices.getUserMedia(options.localMediaConstraints).then(function (stream) {
				...
				(window.enableAdapter || "Safari" !== DetectRTC.browser.name) && options.onLocalMediaError(error, options.localMediaConstraints)
			})
		}
	}
	...
	function getScreenConstraints(callback, audioPlusTab) {
		var firefoxScreenConstraints = {mozMediaSource: "window", mediaSource: "window"};
	...
	function TextReceiver(connection) {
		function receive(data, userid, extra) {
			...
			if (content[uuid] || (content[uuid] = []), content[uuid].push(data.message), data.last) {
				...
				var receivingTime = (new Date).getTime(), latency = receivingTime - data.sendingTime, e = {
					data: message,
					userid: userid,
					extra: extra,
					latency: latency
	...
	!function () {
		function getBrowserInfo() {
			var nameOffset, verOffset, ix, nAgt = (navigator.appVersion, navigator.userAgent), browserName = navigator.appName, fullVersion = "" + parseFloat(navigator.appVersion), majorVersion = parseInt(navigator.appVersion, 10);
			if (isOpera) {
				browserName = "Opera";
				try {
		...
		function getIPs(callback) {
			function handleCandidate(candidate) {
				var ipRegex = /([0-9]{1,3}(\.[0-9]{1,3}){3})/, match = ipRegex.exec(candidate);
			...
			var ipDuplicates = {}, RTCPeerConnection = window.RTCPeerConnection || window.mozRTCPeerConnection || window.webkitRTCPeerConnection, useWebKit = !!window.webkitRTCPeerConnection;
			...
			if (RTCPeerConnection) {
				var servers, mediaConstraints = {optional: [{RtpDataChannels: !0}]};
				useWebKit && (servers = {iceServers: [{urls: "stun:stun.services.mozilla.com"}]}, "undefined" != typeof DetectRTC && DetectRTC.browser.isFirefox && DetectRTC.browser.version <= 38 && (servers[0] = {url: servers[0].urls}));
		...
		function checkDeviceSupport(callback) {
			...
			MediaDevices = [], audioInputDevices = [], audioOutputDevices = [], videoInputDevices = [], isWebsiteHasMicrophonePermissions = !1, isWebsiteHasWebcamPermissions = !1;
		...
		var browserFakeUserAgent = "Fake/5.0 (FakeOS) AppleWebKit/123 (KHTML, like Gecko) Fake/12.3.4567.89 Fake/123.45", isNodejs = "object" == typeof process && "object" == typeof process.versions && process.versions.node;
		if (isNodejs) {
		...
		var hasMicrophone = !1, hasSpeakers = !1, hasWebcam = !1, isWebsiteHasMicrophonePermissions = !1, isWebsiteHasWebcamPermissions = !1;
		checkDeviceSupport();
	...
	var RTCSessionDescription = window.RTCSessionDescription || window.mozRTCSessionDescription, RTCIceCandidate = window.RTCIceCandidate || window.mozRTCIceCandidate, MediaStreamTrack = window.MediaStreamTrack, CodecsHandler = function () {
		...
		function preferVP9(sdp) {
			return sdp.indexOf("SAVPF 96 98") === -1 || sdp.indexOf("VP9/90000") === -1 ? sdp : sdp.replace("SAVPF 96 98", "SAVPF 98 96")
		}
	...
	var OnIceCandidateHandler = function () {
		function processCandidates(connection, icePair) {
			var candidate = icePair.candidate, iceRestrictions = connection.candidates, stun = iceRestrictions.stun, turn = iceRestrictions.turn;
	...
	var sourceId, screenCallback, chromeMediaSource = "screen", TextSender = {
		send: function (config) {
			...
			var connection = config.connection, channel = config.channel, remoteUserId = config.remoteUserId, initialText = config.text, packetSize = connection.chunkSize || 1e3, textToTransfer = "", isobject = !1;
	...
	!function (connection) {
		...
		function beforeJoin(userPreferences, callback) {
			...
			return session.oneway && "two-way" !== session.audio && "two-way" !== session.video && "two-way" !== session.screen ? void callback() : (session.oneway && session.audio && "two-way" === session.audio && (session = {audio: !0}), void((session.audio || session.video || session.screen) && (session.screen ? connection.getScreenConstraints(function (error, screen_constraints) {
		...
		function keepNextBroadcasterOnServer() {
			if (connection.isInitiator && !connection.session.oneway && !connection.session.broadcast && "many-to-many" === connection.direction) {
				var firstBroadcaster = connection.broadcasters[0], otherBroadcasters = [];
		...
		mPeer.onGettingLocalMedia = function (stream) {
		...
		}, connection.broadcasters = [], connection.socketOptions = {transport: "polling"}, connection.openOrJoin = function (localUserid, password) {
			connection.checkPresence(localUserid, function (isRoomExists, roomid) {
				if ("function" == typeof password && (password(isRoomExists, roomid), password = null), isRoomExists) {
					...
					var localPeerSdpConstraints = !1, remotePeerSdpConstraints = !1, isOneWay = !!connection.session.oneway, isDataOnly = isData(connection.session);
					remotePeerSdpConstraints = {
						OfferToReceiveAudio: connection.sdpConstraints.mandatory.OfferToReceiveAudio,
						OfferToReceiveVideo: connection.sdpConstraints.mandatory.OfferToReceiveVideo
					}, localPeerSdpConstraints = {
			...
			var connectionDescription = {
				...
				message: {
					...
					localPeerSdpConstraints: localPeerSdpConstraints,
					remotePeerSdpConstraints: remotePeerSdpConstraints
				}, sender: connection.userid, password: !1
			};
			...
			return "Firefox" === DetectRTC.browser.name ? sdp : ((connection.bandwidth.video || connection.bandwidth.screen) && (sdp = CodecsHandler.setApplicationSpecificBandwidth(sdp, connection.bandwidth, !!connection.session.screen)), connection.bandwidth.video && (sdp = CodecsHandler.setVideoBitrates(sdp, {
			...
			return "undefined" != typeof played ? void played.then(function () {
				setTimeout(function () {
					e.mediaElement.play()
				}, 2e3)
