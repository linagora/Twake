...
a.getServerUrl = function(a, d) {
  return 'file:' === c.location.protocol
    ? d
      ? d
      : 'ws://127.0.0.1/ws'
    : ('https:' === c.location.protocol ? 'wss://' : 'ws://') + c.location.hostname + ('' !== c.location.port ? ':' + c.location.port : '') + '/' + (a ? a : 'ws');
};
    ...
    if (b[1] in e._calls) {
      ...
      if (null === e._session_id) {
        ...
        if (a._debugrpc || a._debugpubsub)
          console.group('WAMP Welcome'), console.info(e._wsuri + '  [' + e._session_id + ']'), console.log(e._wamp_version), console.log(e._server), console.groupEnd();
...
const autobahn_compatible = window.autobahn;
