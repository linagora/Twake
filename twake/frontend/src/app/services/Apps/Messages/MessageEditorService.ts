...
export class MessageEditorService extends Observable {
  ...
  async setContent(threadId: string, messageId: string, content: string): Promise<void> {
    ...
    if (!messageId) {
      const all = ((await LocalStorage.getItem(MessageEditorService.LOCALSTORAGE_KEY)) || {}) as any;
      all[this.getCacheId(threadId)] = [
        content,
        new Date().getTime(),
      ];
  ...
  async getContent(threadId: string = "", messageId: string = ""): Promise<string> {
    ...
    if (!messageId) {
      const all = this.cleanSavedInputContents((await LocalStorage.getItem(MessageEditorService.LOCALSTORAGE_KEY)) || {});
