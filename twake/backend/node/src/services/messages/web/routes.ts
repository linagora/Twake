...
const routes: FastifyPluginCallback<{ service: MessageServiceAPI; realtime: RealtimeServiceAPI }> =
  (fastify: FastifyInstance, options, next) => {
    ...
    const viewsController = new ViewsController(options.realtime, options.service);
